...
function c7576.fil(c,e,tp,mg,lv,lc)
	...
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0xbb) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
		...
		and (c:GetLevel()==lv or (c:GetLevel()<lv and lc>1 and Duel.IsExistingMatchingCard(c7576.fil,tp,LOCATION_DECK,0,1,nil,e,tp,mg2,lv-c:GetLevel(),lc-1)))
end
function c7576.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	lc=math.min(lc,3)
	local g=Duel.GetMatchingGroup(c7576.fil,tp,LOCATION_DECK,0,nil,e,tp,nil,8,lc)
	if chk==0 then return g:GetCount()>0 end
