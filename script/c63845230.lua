...
function c7532.initial_effect(c)
	c:EnableReviveLimit()
	--special summon
	...
	e1:SetCondition(c7532.spcon)
	e1:SetOperation(c7532.spop)
	c:RegisterEffect(e1)
	...
	ea:SetCountLimit(1)
	ea:SetCondition(c7532.rmcon)
	ea:SetTarget(c7532.rmtg)
	ea:SetOperation(c7532.rmop)
	c:RegisterEffect(ea)
end

function c7532.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local g=Duel.GetMatchingGroup(Card.IsAbleToRemoveAsCost,tp,LOCATION_HAND+LOCATION_ONFIELD+LOCATION_EXTRA,0,c)
	return g:GetCount()>=5
end
function c7532.spop(e,tp,eg,ep,ev,re,r,rp,c)
...
function c7532.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local bc=e:GetLabelObject()
	if chk==0 then return bc:IsAbleToRemove() end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,bc,1,0,0)
end
function c7532.rmop(e,tp,eg,ep,ev,re,r,rp)
	local bc=e:GetLabelObject()
