...
function c65976795.initial_effect(c)
	...
	e1:SetTarget(c65976795.destg)
	e1:SetOperation(c65976795.desop)
	c:RegisterEffect(e1)
	--special summon
...
function c65976795.descfil(c)
	return c:IsFaceup() and c:IsSetCard(0x10ed)
end
function c65976795.desfil(c)
	return c:IsFacedown() and c:IsDestructable()
end
function c65976795.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_ONFIELD) and c65976795.desfil(chkc) end
	local ct=Duel.GetMatchingGroupCount(c65976795.descfil,tp,LOCATION_MZONE,0,nil)
	if chk==0 then return ct>0 and Duel.IsExistingTarget(c65976795.desfil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,c65976795.desfil,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,ct,ct,nil)
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,g:GetCount(),0,0)
end
function c65976795.desop(e,tp,eg,ep,ev,re,r,rp)
