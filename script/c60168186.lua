...
function c60168186.initial_effect(c)
	...
	e2:SetDescription(aux.Stringid(60168186,1))
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_SPSUMMON_SUCCESS)
	e2:SetCountLimit(1)
	e2:SetRange(LOCATION_SZONE)
	e2:SetLabel(TYPE_SYNCHRO)
	e2:SetCondition(c60168186.con)
	e2:SetTarget(c60168186.syntg)
	e2:SetOperation(c60168186.synop)
	c:RegisterEffect(e2)
	--Fusion
	local e3=e2:Clone()
	e3:SetDescription(aux.Stringid(60168186,0))
	e3:SetLabel(TYPE_FUSION)
	e3:SetCategory(CATEGORY_RECOVER)
	e3:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e3:SetTarget(c60168186.fustg)
	e3:SetOperation(c60168186.fusop)
	c:RegisterEffect(e3)
	--Xyz
	local e4=e2:Clone()
	...
	e4:SetTarget(c60168186.xyztg)
	e4:SetOperation(c60168186.xyzop)
	c:RegisterEffect(e4)
	--Pendulum
	local e5=e2:Clone()
	...
	e5:SetTarget(c60168186.pentg)
	e5:SetOperation(c60168186.penop)
	c:RegisterEffect(e5)
	--damage
...
function c60168186.cfil(c,tp,ty)
	return c:IsFaceup() and c:IsSetCard(0x10af) and c:IsType(ty) 
		and c:IsControler(tp) and c:IsPreviousLocation(LOCATION_EXTRA)
end
function c60168186.con(e,tp,eg,ep,ev,re,r,rp)
	local ty=e:GetLabel()
	return eg and eg:IsExists(c60168186.cfil,1,nil,tp,ty)
end

function c60168186.syntg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local sg=eg:Filter(c60168186.cfil,nil,tp,TYPE_SYNCHRO)
	Duel.SetTargetCard(sg)
end
function c60168186.synop(e,tp,eg,ep,ev,re,r,rp)
...
function c60168186.fustg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetPlayer(tp)
	Duel.SetTargetParam(1000)
	Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,tp,1000)
end
function c60168186.fusop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	Duel.Recover(p,d,REASON_EFFECT)
end

function c60168186.xyztg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c60168186.xyzop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
...
function c60168186.pentg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1) end
	Duel.SetOperationInfo(0,CATEGORY_HANDES,nil,0,tp,1)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function c60168186.penop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
