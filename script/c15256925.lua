...
function c15256925.initial_effect(c)
	...
	e1:SetCondition(c15256925.rmcon)
	e1:SetTarget(c15256925.rmtg)
	e1:SetOperation(c15256925.rmop)
	c:RegisterEffect(e1)
	--to deck
...
function c15256925.rmcfil(c)
	return c:IsFaceup() and c:IsLevelAbove(8) and c:IsRace(RACE_SPELLCASTER)
end
function c15256925.rmcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c15256925.rmcfil,tp,LOCATION_MZONE,0,1,nil)
end
function c15256925.rmtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c15256925.rmop(e,tp,eg,ep,ev,re,r,rp)
...
function c15256925.spfil(c,e,tp)
...
function c15256925.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(c15256925.spfil,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c15256925.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<1 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c15256925.spfil,tp,LOCATION_DECK,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
