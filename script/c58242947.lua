...
function c58242947.filter(c,e,tp)
	local te,eg,ep,ev,re,r,rp=c:CheckActivateEffect(false,true,true)
	if te==nil then return false end
	local target=te:GetTarget()
	return c:GetType()==0x4 and c:IsAbleToRemoveAsCost() and (not target or target(e,tp,eg,ep,ev,re,r,rp,0))
end
function c58242947.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsAbleToRemoveAsCost()
		and Duel.IsExistingMatchingCard(c58242947.filter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(58242947,1))
	local g=Duel.SelectMatchingCard(tp,c58242947.filter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
