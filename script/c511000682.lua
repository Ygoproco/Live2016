...
function c511000682.filter(c)
	local rk=c:GetRank()
	return c:IsFaceup() and c:IsSetCard(0x48) and not Duel.IsExistingMatchingCard(c511000682.cfilter,c:GetControler(),LOCATION_MZONE,0,1,nil,rk)
end
function c511000682.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c511000682.filter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c511000682.filter,tp,LOCATION_MZONE,0,1,1,nil)
