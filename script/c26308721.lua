...
function c26308721.initial_effect(c)
	...
	e1:SetCondition(c26308721.ctcon)
	e1:SetTarget(c26308721.cttg)
	e1:SetOperation(c26308721.ctop)
	c:RegisterEffect(e1)
	--
	...
	e2:SetCountLimit(1)
	e2:SetTarget(c26308721.tg)
	e2:SetOperation(c26308721.op)
	c:RegisterEffect(e2)
end

function c26308721.ctcon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp
end
function c26308721.cttg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c26308721.ctop(e,tp,eg,ep,ev,re,r,rp)
...
function c26308721.fil(c,lv)
	return c:IsFaceup() and c:IsLevelBelow(lv) and c:IsAbleToChangeControler()
end
function c26308721.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local lv=e:GetHandler():GetLevel()
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c26308721.fil(chkc,lv) end
	if chk==0 then return Duel.IsExistingTarget(c26308721.fil,tp,0,LOCATION_MZONE,1,nil,lv) end
	e:SetLabel(lv)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_CONTROL)
	local g=Duel.SelectTarget(tp,c26308721.fil,tp,0,LOCATION_MZONE,1,1,nil,lv)
	Duel.SetOperationInfo(0,CATEGORY_CONTROL,g,1,0,0)
end
function c26308721.op(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
