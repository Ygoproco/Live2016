...
function c27642961.initial_effect(c)
	...
	e2:SetCondition(c27642961.drcon1)
	e2:SetTarget(c27642961.drtg)
	e2:SetOperation(c27642961.drop)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	e3:SetCode(EVENT_DESTROYED)
	e3:SetCondition(c27642961.drcon2)
	c:RegisterEffect(e3)
	--
	...
	e4:SetTarget(c27642961.sptg)
	e4:SetOperation(c27642961.spop)
	c:RegisterEffect(e4)
end
...
function c27642961.drcon1(e,tp,eg,ep,ev,re,r,rp)
...
function c27642961.drfil(c)
	return c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsReason(REASON_EFFECT)
end
function c27642961.drcon2(e,tp,eg,ep,ev,re,r,rp)
	if not re then return false end
	local rc=re:GetHandler()
	return eg:IsExists(c27642961.drfil,1,nil) and re:IsActiveType(TYPE_MONSTER) and rp==tp and rc and rc:IsSetCard(0xee) and rc:IsControler(tp)
end
...
function c27642961.spfil(c,e,tp)
	return c:IsSetCard(0xee) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c27642961.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(c27642961.spfil,tp,LOCATION_HAND,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
end
function c27642961.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<1 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c27642961.spfil,tp,LOCATION_HAND,0,1,1,nil,e,tp)
