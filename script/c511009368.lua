...
function c511009368.desfilter(c)
	return c:IsDestructable()
end
function c511009368.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) and c511009368.desfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c511009368.desfilter,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c511009368.desfilter,tp,0,LOCATION_MZONE,1,1,nil)
