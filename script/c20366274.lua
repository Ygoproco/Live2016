...
function c20366274.check1(c,mg,sg,chkf)
	local g=mg:Clone()
	if sg:IsContains(c) then g:Sub(sg) end
	return g:IsExists(c20366274.check2,1,c,c,chkf)
end
function c20366274.check2(c,c2,chkf)
	...
	while tc do
		...
		if c20366274.ffilter2(tc) or tc:IsHasEffect(511002961) then g2:AddCard(tc) if aux.FConditionCheckF(tc,chkf) then fs=true end end
		tc=g:GetNext()
	end
...
function c20366274.fuscon(e,g,gc,chkf)
	...
	if gc then
		...
		return c20366274.check1(gc,mg,sg,chkf)
	end
	return mg:IsExists(c20366274.check1,1,nil,mg,sg,chkf)
end
