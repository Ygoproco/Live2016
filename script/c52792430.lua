...
function c52792430.spfil(c,e,tp)
...
function c52792430.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return t and t:IsLevelBelow(a:GetLevel())
		and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(c52792430.spfil,tp,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c52792430.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<1 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c52792430.spfil,tp,LOCATION_DECK,0,1,1,nil,e,tp)
