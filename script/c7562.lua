...
function c7562.initial_effect(c)
	...
	e1:SetCountLimit(1,7562+EFFECT_COUNT_CODE_OATH)
	e1:SetTarget(c7562.target)
	e1:SetOperation(c7562.activate)
	c:RegisterEffect(e1)
end
function c7562.filter(c,e,tp,mg)
...
function c7562.matfilter1(c,tp)
	local lv=c:GetLevel()
	return lv>0 and lv<8 and c:IsAttribute(ATTRIBUTE_LIGHT+ATTRIBUTE_DARK) and c:IsAbleToGrave()
		and Duel.IsExistingMatchingCard(c7562.matfilter2,tp,LOCATION_DECK,0,1,c,lv,c:GetAttribute())
end
function c7562.matfilter2(c,lv,att)
...
function c7562.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local mg=Duel.GetMatchingGroup(c7562.matfilter1,tp,LOCATION_HAND,0,nil,tp)
		return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and Duel.IsExistingMatchingCard(c7562.filter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil,e,tp,mg)
	end
...
function c7562.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc1 then
		...
		local mat=Duel.SelectMatchingCard(tp,c7562.matfilter1,tp,LOCATION_HAND,0,1,1,nil,tp)
		local tc2=mat:GetFirst()
		if not tc2 then return end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
		local mat2=Duel.SelectMatchingCard(tp,c7562.matfilter2,tp,LOCATION_DECK,0,1,1,nil,tc2:GetLevel(),tc2:GetAttribute())
		mat:Merge(mat2)
		tc1:SetMaterial(mat)
