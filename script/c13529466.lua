...
function c13529466.aclimit3(e,tp,eg,ep,ev,re,r,rp)
	if ep==tp or not re:IsActiveType(TYPE_MONSTER) then return end
	e:GetHandler():RegisterFlagEffect(13529466+1,RESET_EVENT+0x3ff0000+RESET_PHASE+PHASE_END,0,1)
end
function c13529466.aclimit4(e,tp,eg,ep,ev,re,r,rp)
	if ep==tp or not re:IsHasType(EFFECT_TYPE_ACTIVATE) then return end
	e:GetHandler():ResetFlagEffect(13529466+1)
end
function c13529466.econ2(e)
	return e:GetHandler():GetFlagEffect(13529466+1)~=0
end
function c13529466.elimit(e,re,tp)
	return re:IsActiveType(TYPE_MONSTER) and not re:GetHandler():IsImmuneToEffect(e)
end
function c13529466.atkcon(e)
	return e:GetHandler():GetFlagEffect(13529466+2)~=0
end
function c13529466.atktg(e,c)
	return c:GetFieldID()~=e:GetLabel()
end
function c13529466.checkop(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():GetFlagEffect(13529466+2)~=0 then return end
	local fid=eg:GetFirst():GetFieldID()
	e:GetHandler():RegisterFlagEffect(13529466+2,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
