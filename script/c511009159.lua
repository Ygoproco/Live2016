...
function c511009159.filter(c)
	return c:IsFaceup()
end
function c511009159.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c511009159.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c511009159.filter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,c511009159.filter,tp,LOCATION_MZONE,0,1,1,nil)
