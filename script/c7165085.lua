	...
	if tc:IsType(TYPE_TRAP) then
		...
		if not te then
			...
			if te:GetCode()==EVENT_FREE_CHAIN and te:IsActivatable(tep)
				...
				Duel.RaiseEvent(Group.CreateGroup(),EVENT_CHAIN_ACTIVATING,te,0,tep,tep,cn)
				Duel.RaiseEvent(Group.CreateGroup(),EVENT_CHAIN_SOLVING,te,0,tep,tep,cn)
				if operation then operation(te,tep,eg,ep,ev,re,r,rp) end
				Duel.RaiseEvent(Group.CreateGroup(),EVENT_CHAIN_SOLVED,te,0,tep,tep,cn)
