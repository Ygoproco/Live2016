...
function c47882565.filter1(c,e,tp)
	...
	return rk>1 and c:IsFaceup() and c:IsSetCard(0xe5)
		and Duel.IsExistingMatchingCard(c47882565.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,c,rk)
end
...
function c47882565.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c47882565.filter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp,tc,tc:GetRank())
	local sc=g:GetFirst()
	if sc then
...
function c47882565.atkval(e,c)
	return Duel.GetMatchingGroupCount(c47882565.atkfil,c:GetControler(),LOCATION_MZONE,0,nil)*500
end
function c47882565.atkfil(c)
