...
function c73055622.filter(c,e,tp,m1,m2,ft)
	...
	if ft>0 then
		...
		return ft>-1 and mg:IsExists(c73055622.mfilterf,1,nil,tp,mg,c)
	end
end
...
function c73055622.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(c73055622.filter,tp,LOCATION_HAND,0,1,nil,e,tp,mg1,mg2,ft)
	end
...
function c73055622.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c73055622.filter,tp,LOCATION_HAND,0,1,1,nil,e,tp,mg1,mg2,ft)
	local tc=g:GetFirst()
	if tc then
