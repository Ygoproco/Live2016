...
function c88069597.initial_effect(c)
	...
	e5:SetCountLimit(1)
	e5:SetCondition(c88069597.mtcon)
	e5:SetOperation(c88069597.mtop)
	c:RegisterEffect(e5)
	--
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(88069597,1))
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_IGNITION)
	e2:SetRange(LOCATION_SZONE)
	e2:SetCountLimit(1)
	e2:SetTarget(c88069597.sptg)
	e2:SetOperation(c88069597.spop)
	c:RegisterEffect(e2)
end
...
function c88069597.mtop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLP(tp)>800 and Duel.SelectYesNo(tp,aux.Stringid(88069597,0)) then
...
function c88069597.spfil(c,e,tp)
...
function c88069597.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetMatchingGroup(c88069597.spfil,tp,LOCATION_HAND+LOCATION_GRAVE,0,nil,e,tp)
...
function c88069597.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c88069597.spfil,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,1,nil,e,tp)
	local tc=g:GetFirst()
