...
function c7613.thcfil(c)
	return c:IsPreviousLocation(LOCATION_MZONE) and c:GetCounter(0x1041)>0
end
function c7613.thcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c7613.thcfil,1,nil)
end
function c7613.thfil(c)
	return c:IsSetCard(0xf3) and c:IsAbleToHand()
end
function c7613.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c7613.thfil,tp,LOCATION_DECK,0,1,nil) end
